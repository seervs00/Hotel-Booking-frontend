...
const AllRooms = () => {
  ...
  const matchPriceRange = (room) => {
    return (
      ...
      selectedFilters.priceRange.some((range) => {
        const [min, max] = range.replace(currency, '').trim().split(' to ').map(Number);
  ...
  const filteredRooms = useMemo(() => {
    return rooms
      .filter((room) => matchesRoomType(room) && matchPriceRange(room) && filterDestination(room))
      .sort(sortRooms);
          ...
          <div key={room.id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0'>
            ...
            <div className='md:w-1/2 flex-col gap-2'>
              ...
              <p
                ...
                className='text-gray-800 cursor-pointer font-semibold hover:text-primary transition'
              >
                {room.hotel.name}
              </p>
              ...
              <div className='flex flex-wrap items-center mt-3 mb-6 gap-4'>
                {room.amenities.map((item, index) => (
                  <div key={index} className='flex items-center gap-2 px-3 py-2 rounded-lg bg-[#F5F5FF]/70'>
                    <img src={facilityIcons[item]} alt={`${item} icon`} className='w-5 h-5' />
                    <p className='text-xs'>{item}</p>
                  </div>
                ))}
              </div>

              <p className='text-xl font-medium text-gray-700 hover:text-primary transition duration-200'>
      ...
      <div className='bg-white w-80 border border-gray-300 text-gray-600 max-lg:mb-8 min-lg:mt-16'>
        <div className={`flex items-center justify-between px-5 py-2.5 min-lg:border-b border-gray-300 ${openFilters && 'border-b'}`}>
          ...
          <div className='text-xs'>
            <span className='lg:hidden cursor-pointer' onClick={() => setOpenFilters(!openFilters)}>
              {openFilters ? 'HIDE' : 'SHOW'}
            </span>
            <span className='hidden lg:block cursor-pointer' onClick={clearFilters}>
              CLEAR
            </span>
          </div>
        </div>
