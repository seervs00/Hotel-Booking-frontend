...
const Hero = () => {
  ...
  const onSearch = async(e) => {
    ...
    if (!trimmedDestination) return toast.error("Please enter a destination");
    ...
    try {
      ...
      const res = await axios.post(
        ...
        { recentSearchedCity: trimmedDestination },
        { headers: { Authorization: `Bearer ${await getToken()}` } }
      );

      if (res.data.success) {
        setSearchedCities((prev) => {
          const filtered = prev.filter((city) => city !== trimmedDestination);
          const updated = [...filtered, trimmedDestination];
  ...
  return (
    <div className='flex flex-col items-start justify-center px-6 md:px-16 lg:px-24 xl:px-32 text-white bg-[url("/src/assets/hotelbg.jpg")] bg-no-repeat bg-cover bg-center h-screen'>
